...
3. Try not to use a separate event handler for each button. With `$(event.currentTarget)` or `$(this)` you could use just one event handler for the numbers by grabbing the text from the clicked element.
