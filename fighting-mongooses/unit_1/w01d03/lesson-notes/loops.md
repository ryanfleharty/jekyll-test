...
<!--Do this all in dev tools, no following along -->
...
The loop will end when `num <= 1000` is false. So we increase the value of `num` by 1 each time the loop runs.
...
<!--Have students share loop in Slack when done -->
...
```javascript
for (initial condition; end condition; repeating expression) {

}
```

1. some initial code supplied to the loop -- usually a numerical starting value of the loop
2. the condition under which the loop runs -- it will run until the expression is false (thus ending the loop)
...
<!--Remember that "almost" (always using for loops)?  Imagine we have a condition like "level is done" in a video game a for loop won't help us much...but once `levelIsDone` is true, we can move on to the next level -->
