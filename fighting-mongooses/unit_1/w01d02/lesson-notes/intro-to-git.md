...
1. History and comparison of versioning tools
...
In the days of old, people used [FTP](https://en.wikipedia.org/wiki/File_Transfer_Protocol), [CSV](https://en.wikipedia.org/wiki/Comma-separated_values), [SVN](https://en.wikipedia.org/wiki/Apache_Subversion), and [Perforce](https://www.perforce.com/).

Another popular cloud solution that is used with git, Github's competitor, is [Bitbucket](https://bitbucket.org/). Yet another is [GitLab](https://gitlab.com/).
...
**Github Enterprise:** this is a way for companies to own the server that their work is on and save some money. The code here is private. You will be using this for your daily classwork and most homework.
...
- If you want to make a copy of a repository on your computer (also known as local) it's called `cloning`
- If you want to make a copy of a repository on Github and store it on your account, that's called `forking` - note: `forking` is specific to Github and is *NOT* a git command
 ...
 \* `pull` can also be used in a request for someone to pull in your code - on Github you may see something called `pull requests`

When you use the terminal you are talking to your computer. There is a structure to the commands you tell it.  
...
- Seriously contemplate starting our project from scratch again
...
For both ways, let's make a folder in our `seiWork` folder called `github_practice`
...
1. Create a new repo by entering a name in the `Repository Name` field. Note that this name has to be *unique* to your repository. Let's use `repo-practice`

    ![repo name](https://i.imgur.com/j8Ex7hX.png)

1. This will be a **public** repo. Private repos require a paid account.

1. Click on "Initialize this repository with a README". A README file is just a blank document that is the default page that your repository displays. It is a markdown file and normally includes information that you provide about your application.
...
1. In your terminal / Git Bash, navigate to the folder where you would like to keep the folder for this repo. Let's use that `github_practice` folder we created in our work folder
...
1. Go ahead and open the `README.md` in your text editor (Bonus! Do it from the command line!). You can write plain text in a markdown file. If you don't know what to write, go with a classic `Hello World!`, then save this file.
...
1. Navigate to the folder where you're keeping the files for this lesson. Likely, you just need to `cd ..` back to the `github_practice` folder.
...
1. Create a new folder called `another_project` inside `github_practice` - make sure you are NOT inside the git repo you created earlier
...
1. Run `touch README.md`

1. Run `atom README.md` and type some text and save the file

1. In terminal, run `git add README.md`, then `git commit -m 'first commit'`

1. We now have a local git repo, but we have to create a repo on Github and connect it. Navigate to Github and log in if you're not logged in already.
...
You'll do lab work, homework, and morning exercises in your local (on your computer) copy.
...
1. Navigate to: https://git.generalassemb.ly/sei-flex/dan-abramov
...
1. Navigate inside the folder where you will keep your work (most likely the `seiWork` folder)
...
1. copy the url using the `clone or download` button and then the clipboard button
...
And, upstream should be associated with the instructor copy. There should be `sei-flex` as part of the URL
...
5. You can change and push your code as many times as you'd like. In fact committing your code often is highly recommended.
...
We have a [walkthrough](../../../../../wiki/Homework-Submission) on how to submit homework in our repo.
...
If you are in the root of your git repository you can type `ls -a` to show your hidden files and you'll see the folder `.git`
...
>**NOTE:** For the first few weeks of class, do NOT run the above command unless you have cleared it with the instructional staff. It has the potential to delete files you will NEVER get back.

## See Which Branch You're On
Some people like seeing what branch they're on or if they are in a git repository by having a message in the terminal prompt. In this case the master branch is listed in green:
